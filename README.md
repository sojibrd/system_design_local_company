...
বাংলাদেশি কোম্পানির কারিগরি আলোচনার system design অংশের প্রস্তুতি: সাতটা system design ডক নিজের প্রজেক্ট দিয়ে লেখা, `srdtube`-এর ছয় সেকশনের design doc, আর চারটা লোকাল প্রশ্ন মুখে — ২৮ দিনে। দিনে একটা ছোট কাজ, ঝালাইসহ, শেখার বিজ্ঞান মেনে। তারপর থামা।
...
- **Rail:** সব পাতায় বাঁয়ে rail থাকে, মোবাইলে drawer। তাতে ৫টা পাতার লিংক, plan-এর gauge আর ব্লকগুলো। শুধু খোলা ব্লকের দিনগুলো দেখায়।
- **দিন (`/day/<nnn>/`) · ব্লক (`/block/<slug>/`):** দিনের কাজ আর দিন বা ব্লক শেষের হ্যাঁ/না। যে দিনে এই সাইটের ঘর নেই, সেখানে কেন ফাঁকা তা লেখা।
- **সূত্রের chip:** কাজে `(ডক ১৩)` থাকলে এই সাইটের ঐ ডকের পাতা; `(sim url-shortener · functional)` থাকলে এই সাইটের simulator-এর ঐ সিস্টেম।
...
- কাজে `(ডক ১৩)` বা `(ডক ১১ · ডক ১৩)` = `guide/`-এর ডক (ফাইল না থাকলে build ভাঙে); `(sim url-shortener · functional)` = simulator-এর সিস্টেম আর লেভেল, তালিকা `app/lib/sources.ts`-এ — `app/lib/simulations/`-এ সিস্টেম বা লেভেল যোগ হলে ওখানেও।
